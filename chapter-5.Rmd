...
wyniki_est_df %>%
  ...
  geom_hline(aes(yintercept = cede), color = "blue", linetype = "dashed") + 
  geom_point(aes(y = me)) +
  facet_grid(country ~ quarter, scales = "free_y", switch = "y") +
  scale_linetype_manual(name = "Source", values = c("red", "blue"), breaks  = c("jvs", "cede")) + 
  #scale_y_continuous(labels = scales::percent) + 
  theme_bw() + 
...
wyniki_est_df %>%
  ...
  mutate(country = countrycode(country, "eurostat","country.name"))  %>%
  filter(days == 20) %>%
  select(country, quarter, JVS =jvs, OJA = cede, trimmed = N, Proposed = med, CV = cv) %>%
  xtable(caption = "gdgg", digits = 1) %>%
